...
def feat_num_try(f_tuple):
	res = [i for i in range(len(f_tuple))]
	return res

def random_shuffle(label, sample):
	random.seed(42)  # so that result is the same every time
	size = len(label)
	for i in range(size):
...
def main():
	...
	fd = open("./model1/%s.fscore"%train_file, 'w')
	for t in f_tuples:
		fd.write("%d: \t%.6f\n"%t)
	fd.close()
	...
	print(fnum_v)
	# print('est_acc', est_acc)

	fnum=fnum_v[est_acc.index(max(est_acc))]
	# print('zuida_acc:', max(est_acc))
	# print('buzhidao', est_acc.index(max(est_acc)))
...
initlog("./model1/%s.select"%train_file)
writelog("start: %s\n\n"%datetime.now())
main()
